...
#(  parameter integer L_WIDTH = 4,
    ...
    reg  signed [Q_WIDTH-1:0] Q_reg [0:N_LEVEL-1][0:0];
    ...
    generate
        if (A_DUR_WIDTH<2) begin
            wire [1:0] A_dur_rand_temp;
            lfsr #(.DATA_WIDTH(2))           rand(.clk(clk), .rst(rst), .seed({A_DUR_WIDTH{2'b00}}), .out0(A_dur_rand_temp));
            assign A_dur_rand = A_dur_rand_temp[0];
        end else begin
...
//                    Q_reg[i][1] <= Q_reg[i][0];
    ...
    generate 
        if (N_LEVEL == 2) begin 
            mux2to1 #(.DATA_WIDTH(Q_WIDTH)) QA_mux0( .in0(Q_reg[0][0]),   .in1(Q_reg[1][0]),
                                                     .sel(A_dur),         .out0(Q_act));  
        end else if (N_LEVEL == 4) begin 
            mux4to1 #(.DATA_WIDTH(Q_WIDTH)) QA_mux0( .in0(Q_reg[0][0]),   .in1(Q_reg[1][0]),
                                                     .in2(Q_reg[2][0]),   .in3(Q_reg[3][0]),
                                                     .sel(A_dur),         .out0(Q_act));  
        end else begin // N_LEVEL == 8
            mux8to1 #(.DATA_WIDTH(Q_WIDTH)) QA_mux0( .in0(Q_reg[0][0]),   .in1(Q_reg[1][0]),
                                                     .in2(Q_reg[2][0]),   .in3(Q_reg[3][0]),
                                                     .in4(Q_reg[4][0]),   .in5(Q_reg[5][0]),
                                                     .in6(Q_reg[6][0]),   .in7(Q_reg[7][0]),
                                                     .sel(A_dur),         .out0(Q_act));  
        end
    endgenerate
