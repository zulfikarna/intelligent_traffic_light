...
## Action : non-cyclic, determine how many level should be decreased
...
def EV(_traffic_level_, _action_, _action_sel_,  _q_matrix_):
    ...
    _green_ = _action_[0]
    _q_max_ = max(max(_q_matrix_[_state_curr_][0]), max(_q_matrix_[_state_curr_][1]), max(_q_matrix_[_state_curr_][2]), max(_q_matrix_[_state_curr_][3]))
    for _lane_ in range(N_LANE):
        for _dec_level_ in range(N_LEVEL):
            if (_q_matrix_[_state_curr_][_lane_][_dec_level_] == _q_max_):
                _greed_lane_ = _lane_
                _greed_dec_ = _dec_level_
                
    if(_action_sel_):
        _action_[0] = _greed_lane_
        _action_[1] = _greed_dec_
    else:
        _action_[0] = randint(0, N_LANE - 1)
        _action_[1] = randint(0, N_LEVEL - 1)

    # Update traffic level
    for _lane_ in range(N_LANE):
        if (_lane_ == _action_[0]):
            if ( _traffic_level_[_lane_] - _action_[1] - 1 > 0):
                _traffic_level_[_lane_]  = _traffic_level_[_lane_] - _action_[1] - 1
            else:
                _traffic_level_[_lane_] = 0
        else:
            if( _traffic_level_[_lane_] + 0.25*(_action_[1] + 1) < N_LEVEL):
                _traffic_level_[_lane_]  = _traffic_level_[_lane_] + 0.25*(_action_[1] + 1)
            else:
...
def UPDATER(_state_curr_, _green_, _action_, _state_next_, _reward_, _q_matrix_):
    _next_Q_ = max(max(_q_matrix_[_state_next_][0]), max(_q_matrix_[_state_next_][1]), max(_q_matrix_[_state_next_][2]), max(_q_matrix_[_state_next_][3]))
...
for episode in range(MAX_EPISODE):
    ...
    for step in range(MAX_STEP):
        ...
        q_matrix[state_curr][action[0]][action[1]] = UPDATER(state_curr, green, action, state_next, reward, q_matrix)
