...
## Action : green for a lane and read for others
...
def EV(_traffic_level_, _action_sel_,  _q_matrix_):
    ...
    if (_action_ == _action_max_):
        _reward_ = 10
    elif (_action_ == _action_min_):
        _reward_ = -10
    else:
        _reward_ = -1
...
plt.legend(loc='best')
plt.title('Cummulative reward (V0)')
plt.ylabel('Average reward')
plt.xlabel('Episode')
plt.savefig("v0_reward_{}_{}.png".format(MAX_STEP, MAX_EPISODE))
plt.show()

# Plotting counter 
plt.plot(cum_counter)
plt.title('Step needed to achive goal (V0)')
plt.ylabel('Step')
plt.xlabel('Episode')
plt.savefig("v0_counter_{}_{}.png".format(MAX_STEP, MAX_EPISODE))
plt.show()

# Plotting state visited 
plt.plot(state_visited)
plt.title('How many times each state is visited (V0)')
plt.ylabel('times')
plt.xlabel('State')
plt.savefig("v0_state_{}_{}.png".format(MAX_STEP, MAX_EPISODE))
plt.show()
